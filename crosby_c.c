...
int main(){
	...
	fq1 = fopen("/scratch/experiment/reads_sim_run_len36bp_frag50bp_err0perc.R1.fastq","r");
        fq2 = fopen("/scratch/experiment/reads_sim_run_len36bp_frag50bp_err0perc.R2.fastq","r");
...
void merge_kmer(struct read_pairs pair, struct PARAMS param ){
	...
	int kmer_index[4][4][4] = {0};		    // use to locate the head for a 3mer	
	...
	for (i=0;i<pair.read1_len-2;i=i+3){
		...
		if(kmer_index[base2code[ch1]][base2code[ch2]][base2code[ch3]] == -1){
			
			kmer_index[base2code[ch1]][base2code[ch2]][base2code[ch3]] = idx;
	...
	*/
	...
	for (i=0;i<SEED;i++){
		...
		int idx = kmer_index[base2code[ch1]][base2code[ch2]][base2code[ch3]];
